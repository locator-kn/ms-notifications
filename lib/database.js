...
fns.getImpressionsByLocationId = (id) => {
    return database.collection('impressions')
    .find({location_id: id.toString()})
    .toArray()//TODO remove duplicates
        .then(res => {
            if (!res || res.length === 0) {
                log.error('No device document found for locationId', {collection: 'impressions', location_id: id});
                throw Error('not found');
            }
            return res;
        });
