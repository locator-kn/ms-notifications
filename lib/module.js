...
fns.notifyFollowerNewLocation = (message, next) => {
    ...
    Joi.validate(message.data, validation.notifyNewLocation, (err, data) => {
        ...
        let message = data.user_name + ' hat eine neue Location erstellt.';
...
fns.notifyNewImpressionSameLoc = (message, next) => {
    Joi.validate(message.data, validation.notifyNewImpressionSameLoc, (err, data) => {
        ...
        log.info('composing push-notifications for new impression at same location');

        let user = db.getUserNameById(data.user_id);
        let location = db.getLocationById(data.location_id);

        Promise.all([user, location])
            .then(values => {
                let userName = values[0];
                let locationName = values[1].title;
                let locationId = data.location_id;

                let messageText = userName + ' hat eine Impression bei ' + locationName + ' gepostet!';

                db.getImpressionsByLocationId(locationId)
                    .then(impressions => {

                        impressions.forEach(imp => {

                            return db.getDeviceByUserId(imp.user_id)
                                .then(reveiverDevices => push.sendPush(reveiverDevices, messageText, 'location', data.location_id));
                        });

                    })
                    .then(db.getDeviceByUserId(values[1].user_id)) //send notification also to location owner
                    .then(token => {
                        push.sendPush(token, messageText, 'location', data.location_id);
                ...
                log.error(err, 'Error executing notify message service');
